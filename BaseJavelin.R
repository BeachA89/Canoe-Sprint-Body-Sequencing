...
FootdfAl <- data.frame(fread(choose.files(default = "C:/Users/aaron.beach/CaptureU/Alex2/Alex Monday/a", caption = "Select Foot File")))
PelvisdfAl <- data.frame(fread(choose.files(default = "C:/Users/aaron.beach/CaptureU/Alex2/Alex Monday/a", caption = "Select Pelvis File")))
ThoraxdfAl <- data.frame(fread(choose.files(default = "C:/Users/aaron.beach/CaptureU/Alex2/Alex Monday/a", caption = "Select Thorax File")))
HanddfAl <- data.frame(fread(choose.files(default = "C:/Users/aaron.beach/CaptureU/Alex2/Alex Monday/a", caption = "Select Hand File")))
...
t = length(FootdfAl$time_s)-1
SampleRateTS <- matrix(0, ncol = 1, nrow = t)
for (i in 1:t) {
  SampleRateTS[i,] <- 1/(FootdfAl$time_s[i+1]-FootdfAl$time_s[i])
}
...
#residual_analysis(ThoraxdfAl$highg_ax_m.s.s, cutoff_range = c(0, 20), sample_freq = 1600,
#                 final_order = 4, interval = 1)
bfF <- butter(4, 5/800, type="low")
bfTacc <- butter(4, 0.1, type="low")
...
# residual_analysis(ThoraxdfAl$"gx_deg.s", cutoff_range = c(0, 20), sample_freq = 1600,
#                   final_order = 4, interval = 0.5)


bfT <- butter(4, 0.01, type="low")
...
#Event <- findpeaks((ThoraxdfAl$XAccfilt), minpeakheight = 100, minpeakdistance = 100)
#ev <-  ggplot() + geom_line(aes(x=ThoraxdfAl$V1, y=(ThoraxdfAl$XAccfilt))) + geom_point(aes(x=Event[,2], y=Event[,1]))

############Plot to find events
ev <-  ggplot() + geom_line(aes(x=ThoraxdfAl$V1, y=(ThoraxdfAl$XAccfilt)))
ggplotly(ev)
Event = c(9914, 10951, 11750, 12607)

EventRelease <- findpeaks((HanddfAl$ResAcc), minpeakheight = 250, minpeakdistance = 1000)
#ev <-  ggplot() + geom_line(aes(x=HanddfAl$V1, y=(HanddfAl$ResAcc))) + geom_point(aes(x=EventRelease[,2], y=EventRelease[,1]))
...
fig <- plot_ly(x = PelvisdfAl$time_s) %>%
  ...
  add_trace(y = HanddfAl$ResAcc, type = 'scatter', mode = 'lines', line = list(color = 'blue'),
