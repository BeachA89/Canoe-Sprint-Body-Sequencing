...
Ballvelocity <-  data.frame(fread(choose.files(default = "C:/Users/aaron.beach/CaptureU/Javelin/Cam/a", caption = "Select Ball Velocity File")))


t = length(LFootdfAl$time_s)-1
SampleRateTS <- matrix(0, ncol = 1, nrow = t)
for (i in 1:t) {
  SampleRateTS[i,] <- 1/(LFootdfAl$time_s[i+1]-LFootdfAl$time_s[i])
}
...
# 
# # PelvisdfAl$z_deg2 = PelvisdfAl$z_deg-LFootdfAl$z_deg+10
# # ThoraxdfAl$z_deg2 = ThoraxdfAl$z_deg-LFootdfAl$z_deg+8
# 
...
#PelvisdfAl$ResAccfilt <- filtfilt(bfF, PelvisdfAl$ResAcc)
#ThoraxdfAl$ResAccfilt <- filtfilt(bfF, ThoraxdfAl$ResAcc)

#bfTacc <- butter(4, 5/800, type="low")

#ThoraxdfAl$XAccfilt <- filtfilt(bfTacc, ThoraxdfAl$highg_ax_m.s.s)


 spectrum(PelvisdfAl$"gx_deg.s", method = "ar")
residual_analysis(PelvisdfAl$"gx_deg.s", cutoff_range = c(0, 100), sample_freq = 1600, final_order = 4, interval = 0.5)
...
ev <-  ggplot() + geom_line(aes(x=RFootdfAl$time_s, y=PelvisdfAl$"gx_deg.s")) + geom_line(aes(x=RFootdfAl$time_s, y=PelvisdfAl$gyroxfilt, colour = "red"))
ggplotly(ev)
...
residual_analysis(ThoraxdfAl$"gx_deg.s", cutoff_range = c(0, 100), sample_freq = 1600, final_order = 4, interval = 0.5)
...
ev <-  ggplot() + geom_line(aes(x=RFootdfAl$time_s, y=ThoraxdfAl$"gx_deg.s")) + geom_line(aes(x=RFootdfAl$time_s, y=ThoraxdfAl$gyroxfilt, colour = "red"))
ggplotly(ev)
...
# plot(PelvisdfAl$gyroxfilt, type = 'l')
# 
# plot(HanddfAl$"gx_deg.s", type = 'l')
# plot(HanddfAl$"gy_deg.s", type = 'l')
# plot(HanddfAl$gyroxfilt, type = 'l')
# plot(HanddfAl$gyroyfilt, type = 'l')
# plot(HanddfAl$ResAccfilt, type = 'l')
# 
...
# LFootdfAl$frames <- c(1:length(LFootdfAl$time_s))
# 
# 
# LFootdfAlTrim = LFootdfAl[DStart:DEnd,]
...
ev <-  ggplot() + geom_line(aes(x=LFootdfAl$time_s, y=(LFootdfAl$ResAccfilt)))
ggplotly(ev)

LFmax = 4.093

ev <-  ggplot() + geom_line(aes(x=RFootdfAl$time_s, y=(RFootdfAl$ResAccfilt)))
ggplotly(ev)
...
AllEvents = c(RFmax, LFmax, Release)



#######CREATE EVENTS FROM  ACC PEAKS#########
EventL <- findpeaks((LFootdfAl$ResAccfilt), minpeakheight = 250, minpeakdistance = 100)
ev <-  ggplot() + geom_line(aes(x=LFootdfAl$V1, y=(LFootdfAl$ResAccfilt))) +geom_point(aes(x=EventL[,2], y=EventL[,1]))#+ geom_line(aes(x=PelvisdfAl$V1, y=(PelvisdfAl$highg_ax_m.s.s), colour = "red"))
ggplotly(ev)


EventL <- EventL[,2]
EventL <- sort(EventL, decreasing = FALSE)
EventL <- EventL[2:4]
EventL2 <- LFootdfAl$time_s[EventL]
Release = 5.9525



EventR <- findpeaks((RFootdfAl$ResAccfilt), minpeakheight = 150, minpeakdistance = 100)
ev <-  ggplot() + geom_line(aes(x=RFootdfAl$V1, y=(RFootdfAl$ResAccfilt))) +geom_point(aes(x=EventR[,2], y=EventR[,1]))#+ geom_line(aes(x=PelvisdfAl$V1, y=(PelvisdfAl$highg_ax_m.s.s), colour = "red"))
ggplotly(ev)


EventR <- EventR[,2]
EventR <- sort(EventR, decreasing = FALSE)
EventR <- EventR[1:3]
EventR2 <- RFootdfAl$time_s[EventR]


AllEvents = c(EventL2,EventR2,Release)
...
#if(which.min(LFootdfAl$AccFilt[1:Event[1]]) >0){
#  Event <- Event[-1]
#}

#ev <-  ggplot() + geom_line(aes(x=LFootdfAl$V1, y=(LFootdfAl$AccFilt))) + geom_point(aes(x=Event, y=c(1)))
#ggplotly(ev)

# zero <- NULL
# cycle <- NULL
# cycle[1] = tail((which(LFootdfAl$AccFilt[1:Event[1]] <0)), n=1)
# 
# 
# for (r in 1:(length(Event))){
#   t = r+1
#   # framer = Event[r,2] - LFootdfAl$V1[1]
#   # framet = Event[t,2] - LFootdfAl$V1[1]
#   
#   tryCatch((cycle[t] = tail((which(LFootdfAl$AccFilt[Event[r]:Event[t]] <0)), n=1) + Event[r]),error = function(e) print(NA))
# }




#evB <-  ggplot() + geom_line(aes(x=LFootdfAl$V1, y=(LFootdfAl$AccFilt))) + geom_vline(xintercept=cycle, colour = "red")
#ggplotly(evB)
...
###########################
yaxisrange = range(PelvisdfAl$gyroxfilt)
...
yaxisrange = range(PelvisdfAl$gyroxfilt,ThoraxdfAl$gyroyfilt)
xaxisrange = length(PelvisdfAl$gyroxfilt)
...
fig <- plot_ly(x = PelvisdfAl$time_s) %>%
  ...
  add_trace(y = ThoraxdfAl$gyroxfilt, type = 'scatter', mode = 'lines', line = list(color = 'green'),
            showlegend = TRUE, name = 'Thorax Rotation')%>% 
  add_trace(y = HanddfAl$gyroyfilt/2, type = 'scatter', mode = 'lines', line = list(color = 'blue'),
            showlegend = TRUE, name = 'Hand Rotation')%>% 
  add_trace(y = HanddfAl$ResAccfilt, type = 'scatter', mode = 'lines', line = list(color = 'pink'),
            showlegend = TRUE, name = 'Hand Acc')%>%
# add_trace(y = ThoraxdfAl$ResAccfilt, type = 'scatter', mode = 'lines', line = list(color = 'orange'),
#           showlegend = TRUE, name = 'Thorax acc')%>%
# add_trace(y = PelvisdfAl$ResAccfilt, type = 'scatter', mode = 'lines', line = list(color = 'pink'),
#           showlegend = TRUE, name = 'Thorax acc')%>%
add_trace(x=Ballvelocity$Time, y = Ballvelocity$BallVelocity, type = 'scatter', mode = 'lines', yaxis = "y2", line = list(color = 'purple'),
          showlegend = TRUE, name = 'Jav Velocity')
...
line2[["x0"]] <- AllEvents[3]
line2[["x1"]] <- AllEvents[3]
line2[["y0"]] <- yaxisrange[1]
line2[["y1"]] <- yaxisrange[2]
lines2 <- c(lines2, list(line2))
...
# lines2 <- list()
# line2[["x0"]] <- AllEvents[5]
# line2[["x1"]] <- AllEvents[5]
# line2[["y0"]] <- yaxisrange[1]
# line2[["y1"]] <- yaxisrange[2]
# lines2 <- c(lines2, list(line2))
...
fig <-  layout(fig, xaxis = list(title = "Time (s)",
               ...
               annotations = list(text = c("Foot Strikes", "Release"),  x = c(AllEvents[3], AllEvents[7]), y = c(-400,-400),showarrow=FALSE,xanchor = c('right', 'left'), font = list(size = 24))
) %>%
  config(displayModeBar = T)
...
lines2 <- list()
  ...
  line2[["x0"]] <- AllEvents[7]
  line2[["x1"]] <- AllEvents[7]
  line2[["y0"]] <- yaxisrange[1]
  line2[["y1"]] <- yaxisrange[2]
  lines2 <- c(lines2, list(line2))
